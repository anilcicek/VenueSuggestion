...
public class MostPopularCategoryByTestLocation {
	...
	public List<VenueModel> SuggestVenueByTestLocation(String userId) throws Exception
	{
		...
		Statement statement5= dbConnection.getConn();
    	Statement statement6 = dbConnection.getConn();
		...
		categories=statement2.executeQuery(kategorigetir);
		int j=0;
		while(categories.next() && j<1)
		{
			
			
			while(suggestlocations.next())
			{
				mekanGetir="select top 100   p.VenueId,SQRT(SQUARE(p.Latitude-"+suggestlocations.getString(1)+")+SQUARE(p.Longitude-"+suggestlocations.getString(2)+" ))  AS A , p.Latitude,p.Longitude,p.NeighborhoodCode  from PolsUpdated p  where VenueCategory='"+categories.getString(1) +"' ORDER BY A ASC";
				...
				while(allVenues.next())
				{
			    	  ...
			    	  countcheckin =statement5.executeQuery(sql_3);// herbir mekanın checkin sayısı bulundu
			    	  countcheckin.next();
						...
						venue.setNeighborhoodCode(allVenues.getString(5));
						venuelist.add(venue);
	        	...
	        	while(k>venueliste.size()-2)
	        	{
	        		...
	        		//System.out.println("VenueID:"+venueliste.get(i).getVenueId()+ " Checkin Sayısı:"+venueliste.get(i).getCheckinCount());
	            		...
	            		else
	            		{
	            		   ...
	            		   finalReport = statement6.executeQuery(query);
	            			if(finalReport.next()){
